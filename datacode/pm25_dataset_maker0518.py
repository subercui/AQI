...
class Pm25Dataset(object):
    ...
    '''
    def __init__(self,lon=np.hstack((np.array([116.3883,117.20,121.48,106.54,118.78,113.66]),110+10*np.random.rand(94))),lat=np.hstack((np.array([39.3289,39.13,31.22,29.59,32.04,34.76]),32+10*np.random.rand(94))),start='2015040108',stop='2015051008',d_in=120,d_out=24):
    ...
    def generateinput(self):
        ...
        for i in range(1,self.n_perpoint):#填上矩阵中，剩余trainning example的数据
            ...
            p_time=current+datetime.timedelta(hours=21)
            #p_time就代表这一帧的时间，尝试打开对应文件
            if p_time.hour==3 or p_time.hour==9 or p_time.hour==15 or p_time.hour==21:#去开003的数据
                name=p_time-datetime.timedelta(hours=3)
                filename=name.strftime('%Y%m%d')+'_'+name.strftime('%H')+'_003.pkl.gz'
            ...
            if os.path.exists(gfsdir+filename) and os.path.getsize(gfsdir+filename)>0:#判断文件是否存在
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                    temp=cPickle.load(f)
                    for k in range(self.n_points):
                        inputs[(i+k*self.n_perpoint,90+cnt)]=interp(temp.reshape((180*4+1,360*4)),self.lat_x[k],self.lon_y[k])
                        #(h+24)*2+cnt是用来找对应的格位置，（h+24）*2是对应小时的6格开始位置，再加上cnt作为偏置
                    cnt=cnt+1
                f.close()
            else:#用三小时之前的替换
...
if __name__ == '__main__':
    ...
    savefile(obj.input_data,savedir+'Pm25Dataset0526_t0401-0510_p6.pkl.gz')
    np.savetxt(savedir+"Pm25Dataset0526_t0401-0510_p6.txt", obj.input_data, fmt='%.2f')
    np.random.shuffle(obj.input_data)
    savefile(obj.input_data,savedir+'Pm25Dataset0526_t0401-0510_p6shuffled.pkl.gz')
